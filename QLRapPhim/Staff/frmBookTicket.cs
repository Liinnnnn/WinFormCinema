    ...
    public partial class frmBookTicket : Form
    {
        ...
        public void LoadSchedule()
        {
            DataTable table = data.ReadDatabase("select sr.RoomName, film.Name, st.ShowtimeHour, 32-(select count(TicketID) " +
                    ...
                    "join tblShowRoom sr on sr.RoomID = st.RoomID where sr.CinemaID = '" + cinemaID + "' and CAST(st.Showtime as date) = '" + dtpNgayChieu.Value.ToString("yyyy-MM-dd") + "'");
        ...
        private void frmBookTicket_Load(object sender, EventArgs e)
        {
            ...
            //dgvLichChieu.DataSource = table;
            DataTable table1 = data.ReadDatabase("select sr.RoomName, film.Name, st.ShowtimeHour, 32-(select count(TicketID) " +
                    "from tblTicket where ShowtimeID = st.ShowtimeID ) as Status from tblShowtime st join tblFilm film on film.FilmID = st.FilmID " +
                    "join tblShowRoom sr on sr.RoomID = st.RoomID where sr.CinemaID = '" + cinemaID + "' and CAST(st.Showtime as date) = '" + dtpNgayChieu.Value.ToString("yyyy-MM-dd") + "'");
            dgvLichChieu.DataSource = table1;
            dgvLichChieu.Columns[0].HeaderText = "Tên Phòng Chiếu";
            dgvLichChieu.Columns[1].HeaderText = "Tên Phim";
            dgvLichChieu.Columns[2].HeaderText = "Giờ Chiếu";
            dgvLichChieu.Columns[3].HeaderText = "Số Ghế Trống";
            ...
            DataTable table2 = data.ReadDatabase("select distinct f.Name from tblShowtime st join tblFilm f on st.FilmID = f.FilmID where CAST(st.Showtime as date) = '" + dtpNgayChieu.Value.ToString("yyyy-MM-dd") + "' and st.CinemaID = '" + cinemaID + "'");
            for (int i = 0; i < table2.Rows.Count; i++)
            {
                cbbPhim.Items.Add(table2.Rows[i]["Name"]);
            }
        }
        ...
        private void dgvLichChieu_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                DataTable tb = data.ReadDatabase("select * from tblShowtime st join tblFilm film on film.FilmID = st.FilmID where st.CinemaID = '" + cinemaID + "' and film.Name = N'" + selectedRow.Cells["Name"].Value.ToString() + "' and CAST(st.Showtime as date) = '" + dtpNgayChieu.Value.ToString("yyyy-MM-dd") + "'");
                frmSeatOptions frm = new frmSeatOptions(selectedRow, dtpNgayChieu.Value.ToString("yyyy-MM-dd"), time, selectedRow.Cells["Name"].Value.ToString(), staffID, cinemaID, tb.Rows[0]["ShowtimeID"].ToString());
                frm.ShowDialog();
                this.Close();
            }
        }
